...
// CORS configuration
const corsOptions = {
  origin: '*', // Allowssss all origins, replace with specific origins for security
  methods: ['GET', 'POST', 'OPTIONS'], // Allow specific methods
  allowedHeaders: ['Content-Type', 'Authorization'], // Allow specific headers
  credentials: true, // Set to true if cookies are required
  preflightContinue: false, // Pass preflight response to next middleware
  optionsSuccessStatus: 204 // For legacy browsers that require 204 status
};

// Apply CORS globally to all routes
app.use(cors(corsOptions));

// Setup for the rest of the app
...
const limiter = rateLimit({
  ...
  max: 125, // Limit requests per window
});

const socketLimiter = new RateLimiterMemory({
  points: 5,
  duration: 3, // 5 points per 3 seconds
});

const io = new Server(httpServer, {
  cors: {
    origin: '*', // Allow all origins
    methods: ['GET', 'OPTIONS', 'POST'], // Allow specific methods
  }
});
